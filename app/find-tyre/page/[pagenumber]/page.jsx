...
import { Float, MainButton, PageEnd } from "@/app/components/pageCommon/pageCommon";
...
export default function FindTyre({params}) {
  ...
  const fetchData = async (query) => {
    try {
      const response = await fetch(getStrapiMedia(`/api/products?pagination[pageSize]=${pageSize}&pagination[page]=${currentPage}&sort[0]=premium:desc&filters[segment][name][$eq]=${activeTab}${query}`));
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        if(searchParams.toString()){
          if(pageData.data?.attributes.categories.data[activeTabIndex].attributes.filters.length > 4){
            if(searchParams.has("sub_segment")) checkURLParameter[0] = searchParams.get("sub_segment").replace("+", " ").replace("%2F", "/");
            if(searchParams.has("machinery")) checkURLParameter[1] = searchParams.get("machinery").replace("+", " ").replace("%2F", "/");
            if(searchParams.has("rim_recommended")) checkURLParameter[2] = searchParams.get("rim_recommended").replace("+", " ").replace("%2F", "/");
            if(searchParams.has("size")) checkURLParameter[3] = searchParams.get("size").replace("+", " ").replace("%2F", "/");
            if(searchParams.has("pattern_type")) checkURLParameter[4] = searchParams.get("pattern_type").replace("+", " ").replace("%2F", "/");
          }else{
            if(searchParams.has("machinery")) checkURLParameter[0] = searchParams.get("machinery").replace("+", " ").replace("%2F", "/");
            if(searchParams.has("rim_recommended")) checkURLParameter[1] = searchParams.get("rim_recommended").replace("+", " ").replace("%2F", "/");
            if(searchParams.has("size")) checkURLParameter[2] = searchParams.get("size").replace("+", " ").replace("%2F", "/");
            if(searchParams.has("pattern_type")) checkURLParameter[3] = searchParams.get("pattern_type").replace("+", " ").replace("%2F", "/");
          }
        }
                                      ...
                                      {filter?.items &&
                                        <select name={categories.attributes.name+"-"+filter.title.replace("Select ", "")}
                                          ...
                                          {filter.items.map((item) => (
                                            <option key={item.id} value={item?.value?item.value:''} selected="">
                                              {item?.name}
                                            </option>
                                          ))}
                                          {filtersArray && filtersArray.length > 0 && filtersArray[index].sort().map((item) => (
                                            <option key={item.id} value={item} selected={selectURLParameter[index] == item ? "selected":""}>
                                              {item}
                                            </option>
                                          ))}
                                        </select>
