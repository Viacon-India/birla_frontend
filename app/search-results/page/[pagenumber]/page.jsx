  ...
  const fetchData = async (query) => {
    try {
      const productsResponse = await fetch(
        getStrapiMedia(
          `/api/products?pagination[pageSize]=${pageSize}&pagination[page]=${currentPage}&sort[0]=premium:desc&sort[1]=id:asc${query}`
        )
      );
      const products = await productsResponse.json();
      const pagesResponse = await fetch(
        getStrapiMedia(
          `/api/pages?pagination[pageSize]=${pageSize}&pagination[page]=${currentPage}&sort[0]=id:asc${query}`
        )
      );
              ...
              {searchData.map(
                (product) =>
                  product?.permalink && (
                    ...
                    >
                      {product?.segment ? (
                        <h3 className="text-secondary text-[24px] lg:text-[28px] 2xl:text-[32px] font-bold leading-[1] mb-3">
                      ...
                      <p className="text-primary text-[18px] lg:text-[20px]">
                        {window.location.hostname + product.permalink}
                      </p>
                      {product?.meta_description && (
                        <p className="text-[#4F5662] text-[14px] md:text-[15px] lg:text-[16px] 2xl:text-[17px] line-clamp-4 pt-3">
                          {product.meta_description}
                        </p>
                      )}
                    </Link>
