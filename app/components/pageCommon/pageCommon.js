        ...
        <div className="relative">
          ...
          {Banner?.data ? (
            ...
            <video className="absolute top-0 w-full h-full object-cover" loop autoPlay muted>
              <source src={getStrapiMedia(Banner.data.attributes.url)} type="video/mp4" />
            </video> :
            <Image src={getStrapiMedia(Banner.data.attributes.url)} width={Banner.data.attributes.width} height={Banner.data.attributes.height} alt={Banner.data.attributes?.alternativeText ? Banner.data.attributes.alternativeText: "Hero-Banner" } className="absolute top-0 w-full h-full object-cover" />
          ) : (
            <Image src={StaticBanner} alt="Hero-Banner" className="absolute top-0 w-full h-full object-cover" />
          )}
        </div>
      </div>
    </section>
...
export function PageEnd({
  ...
  return(
    <section className={cn("relative flex pt-10 md:pt-[60px] 2xl:pt-[100px] overflow-hidden ", Background ? Background : "bg-white")} >
      {EndImage?.data ? (
        <Image src={getStrapiMedia(EndImage.data.attributes.url)} width={EndImage.data.attributes.width} height={EndImage.data.attributes.height} alt={EndImage.data.attributes?.alternativeText ? EndImage.data.attributes.alternativeText: "Hero-Banner" } className="absolute left-0 h-[200px] w-[60%]" />
      ) : (
        <Image src={EndStaticImage} alt="bottom-img" className="absolute left-0 h-[200px] w-[60%]" />
      )}
      {/* {EndImageSrc && (
        <Image
          src={EndImageSrc}
