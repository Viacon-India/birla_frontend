                    ...
                    {files.collection.length > 0 &&
                        <div className="grid grid-cols-2 gap-6">
                            {files.collection.map((collection) => ( collection.file?.url &&
                                <Link href={getStrapiMedia(collection.file.url)} className="flex gap-3 items-center w-fit underline underline-offset-2 hover:text-primary hover:decoration-primary" key={collection.id} target="_blank">
